...
**Logging posture.** Setup-link tokens were already redacted by the `pino-http` serializer; QR data URIs and `otpauth://` URIs are returned only in the JSON body of authenticated/pending requests and are never logged. Recovery codes are returned exactly once and stored only as bcrypt hashes (per-row salt, cost 10) — never in plaintext. Admin-issued TOTP resets write `totp_reset` audit rows; user-initiated enroll/disable/regen write `totp_enrolled` / `totp_disabled` / `recovery_codes_regenerated`; successful authenticator-app sign-ins write `totp_used`; logins that consumed a recovery code write `recovery_code_used` so misuse can be reviewed.
